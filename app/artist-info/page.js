...
async function fetchSingleArtist(artistName) {
    ...
    let response = await fetch(`https://ws.audioscrobbler.com/2.0/?method=artist.getinfo&artist=${artistName}&api_key=51de025812af79cb70f4a872936181a0&limit=1&format=json`, {Method: 'POST',  cache: 'no-store' });
  ...
  async function fetchArtistAlbums(artistName) {
    ...
    let response = await fetch(`https://raw.githubusercontent.com/cathysunkc/cprg306-finalproject/master/app/data/album/${artistName.replace(' ', '').replace(',', '')}.json`);
